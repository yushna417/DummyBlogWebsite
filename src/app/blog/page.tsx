...
const getCommentsByPostId = (postId: number) =>
 ...
 const handleRemovePost = (id: number) => {
  ...
  return (
    <main className="lg:px-8 px-5 py-28 flex flex-col">
        ...
        <ul className="inline-flex -space-x-px text-sm">
          <li>            
            <button  className={`flex items-center justify-center lg:px-3 px-[7px]  lg:h-8 h-5 ms-0 leading-tight lg:font-medium font-normal lg:text-sm text-xs border border-e-0 border-gray-300 lg:rounded-s-lg rounded-s-sm hover:bg-gray-100 hover:text-gray-700 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white ${
                currentPage === 1
                  ?" bg-gray-100 border-gray-300  dark:bg-gray-800  text-gray-700 cursor-not-allowed": "text-gray-500 bg-white dark:bg-gray-800 "}`}
            ...
            disabled= {currentPage === 1}>Previous</button>
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((no) => (
                ...
                currentPage === no ?" bg-gray-100 border-gray-300  dark:bg-gray-700  text-white  cursor-not-allowed": "text-gray-500 bg-white dark:bg-gray-800 "}`}>
                      {no} 
                    </button>
                  </li>
                ))}
      ...
      <ul className="grid lg:grid-cols-4 grid-cols-1 gap-4 mx-auto my-10  ">
        {currentPosts.map((post, index) => {  
          ...
          return (      
          <div key={index} className=" p-3 h-44 bg-white flex flex-col justify-between border-4 border-yellow-100 rounded-lg shadow-md dark:bg-gray-700 dark:border-gray-700 ">
              ...
              <div className=" flex gap-x-3 items-center  ">
                 {user && (
                <p className="text-xs flex text-gray-300 items-center gap-x-1 w-24 break-words  ">
                  <FaUser size={14} /> <span className="truncate">{(user.name).split(' ').slice(0, -1).join(' ')}</span>
                </p>
              )}
              {postComments && (
                  <p className="text-xs flex text-gray-300 items-center gap-x-1 leading-tight "><BiSolidCommentDetail size={16}/> {postComments.length} </p> 
              )}
